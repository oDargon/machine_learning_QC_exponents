...
from exponent_opt_tools import local_exponent_removal_suggestion_mixed
...
def evaluate_population_objective_multifile(
    ...
    if len(templates) != 3:
        raise ValueError(f"Number of templates must be equal to exactly 3, you inserted {len(templates)}")
    ...
    for i in range(len(exponent_vectors)):
        E1 = manager.jobs[3*i].exponent_set.energy
        E2 = manager.jobs[3*i+1].exponent_set.energy
        E3 = manager.jobs[3*i+2].exponent_set.energy

        tot  = E1 if E1 is not None else 1e6
        tot += E2 if E2 is not None else 1e6
        tot += E3 if E3 is not None else 1e6

        energies.append(tot/3)
...
def cma_fixed_exponent_count_mixed(
    ...
    *,
    ...
    for gen in range(max_generations):
        ...
        population = es.ask()
        energies   = evaluate_population_objective_multifile(
    ...
    *,
    ...
    with open(log_file, "a") as log_f:
        ...
        if optimize_initial:
            ...
            optimized_init_exp, es = cma_fixed_exponent_count_mixed(current_exp, init_config, opt0_dir,template_paths, generation_size=generation_size, sigma=sigma,
        ...
        for i in range( exponents_to_cull ):
            ...
            cull_exp, idx = local_exponent_removal_suggestion_mixed( current_exp, culling_manager, template_paths, threads=molcas_threads )
            ...
            optimized_exp, es = cma_fixed_exponent_count_mixed( cull_exp, copy.deepcopy(config), opt_dir, template_paths, generation_size=generation_size, sigma=sigma,
                                                    max_generations=max_generations, molcas_threads=molcas_threads, overwrite=overwrite_gens, logging = True if logging >1 else False )
