...
# machine-readable provenance line, first row of every CSV. cbs_target.py reads it
# so the atom / generator / M need not be restated by hand downstream.
META = (f"#META atom={exp.atom_name} generator={GENERATOR} M={M_PARAMS} "
        f"n_step={N_STEP} use_contraction={USE_CONTRACTION}")
...
def shell_worker(shell):
    ...
    for N in n_values:
        codec = from_registry(GENERATOR, m=M_PARAMS, n=N)
