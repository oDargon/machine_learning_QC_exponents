...
MAX_FRAC   = 0.05
SEED       = 42
TOP_FRAC   = 0.10   # fraction of best samples used for the top-k Spearman
...
def top_pairs(pairs, frac=TOP_FRAC):
    """Return the top frac% of pairs sorted by lowest (best) uncontracted energy."""
...
# ── Run 2: self-contracted at each point using that point's own contraction ───
...
# ── Run 3: frozen-contraction (base) at each sample point ────────────────────
...
emit("\n" + "─" * W)
emit(f"  {'#':>4}  {'Uncontr':>18}  {'Self-contr':>18}  {'Fixed-contr':>18}")
emit("─" * W)
for s in samples:
    eu = f"{s['e_uncontr']:.8f}"     if s["e_uncontr"]     is not None else "  FAILED  "
    es = f"{s['e_self_contr']:.8f}"  if s["e_self_contr"]  is not None else "  FAILED  "
    ef = f"{s['e_fixed_contr']:.8f}" if s["e_fixed_contr"] is not None else "  FAILED  "
    emit(f"  {s['i']:>4}  {eu:>18}  {es:>18}  {ef:>18}")
...
out_path = SUBMIT_DIR / "three_way_rank_test.json"
