    ...
    for i in range(CYCLES):
        ...
        if applied_idx is not None:
            ...
            if fat_job.status == Job_Status.COMPLETED and fat_job.exponent_set.resulting_contraction is not None:
                ...
                current_energy = fat_job.exponent_set.energy

                msg = f"[Fat] run launched at cycle {entry['launch_cycle'] + 1} landed -> global contraction refreshed, E = {current_energy:.10f} Eh"
            else:
